...
try:  # Optional convenience during local development
...
def query(q: QueryIn):
    ...
    if question_clean in GREETINGS or question_clean.rstrip('!?.') in GREETINGS:
        return {"answer": "Bonjour ! Je suis FarmLink, ton copilote agricole. N'hésite pas à me poser une question sur les sols, les cultures, l'irrigation, la mécanisation ou les politiques agricoles.", "contexts": []}
...
def build_prompt(question: str, contexts: List[Dict]) -> str:
    if not contexts:
        return (
            ...
            "Aucune information n'est disponible. Invite l'utilisateur a reformuler."
        )
    ctx_block = "\n\n".join(
    ...
    return (
        "Tu es FarmLink, assistant RAG specialise en agriculture.\n"
        "Reponds uniquement avec les informations du CONTEXTE. Si ce n'est pas couvert, dis-le clairement.\n"
        f"Question: {question}\n\n"
        "CONTEXTE:\n"
        f"{ctx_block}\n\n"
        "Reponse detaillee, claire et structuree avec une courte synthese finale et les sources citees en fin de message."
    )
